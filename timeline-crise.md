...
| Jour/Heure | Événement | Actions | Propriétaire | Statut | Détails |
| --- | --- | --- | --- | --- | --- |
| J-1 10:00 | KYCPro admet une intrusion : "Une faille dans notre API (CVE-2026-4242) a été exploitée." | - SOC : Demande un rapport technique à KYCPro. 
- Juridique : Préparation d’un contrat de confidentialité pour l’échange d’infos. | SOC + KYCPro + Juridique | ⚠️ Confirmation partielle. | CVE fictive : CVE-2026-4242 (faille d’authentification dans KYCPro). |
| J-1 12:00 | Preuves d’exfiltration : Le SOC détecte des requêtes POST anormales vers un serveur externe (IP: 45.67.89.12). | - Forensic : Analyse des payloads → Données exfiltrées : IBAN, emails, SSN (5 000 clients). 
- Classification : Incident majeur (DORA Art. 3). | SOC + RSSI | 🔴 Incident critique suspecté. | Outils : Zeek (analyse réseau), Volatility (mémoire). Incident critique suspecté mais qualification formelle non actée : périmètre forensic incomplet, instance dirigeante non encore réunie. Délai de qualification assumé jusqu'à obtention d'éléments suffisants (nombre clients, nature données, impact opérationnel). Documenté en cellule de crise. |
| J-1 14:00 | Réunion SOC + RSSI + Juridique : "Fuite de données confirmée via KYCPro." | - Décision : Isoler KYCPro de l’API Open Banking. 
- Notification interne : Alerte au COMEX et à la Communication. 
- Préparation : Cellule de crise activée. | SOC + RSSI + Juridique | ✅ Isolation KYCPro. | Action concrète : Désactivation du token API de KYCPro. |
| J-1 16:00 | Revendication par "Not42" : Message sur Telegram : "BanqueConnect, vos données KYC sont entre nos mains. #Not42" + échantillon de données (10 IBAN). | - Vérification : Le SOC confirme que les IBAN correspondent à des clients BanqueConnect. 
- Classification : Incident majeur + menace de fuite publique. | SOC + Juridique | 🔴 Crise médiatique imminente. | Preuve : Capture d’écran du message Telegram. Action : Ne pas répondre à Not42 (recommandation ANSSI). |


A J-1 10:00, KYCPro confirme la comprimission officiellement. A J-1 12H00, le SOC et le RSSI confirme que les données de BanqueConnect sont impactées, l'incident critique est suspecté. Une cellule de crise est mise en place mais l'évènement n'est pas classé encore comme majeur / significatif par une entité officielle.

A J-1 14:00 la cellule de crise décide de la première action de mitigation.
...
| Jour/Heure | Événement | Actions | Propriétaire | Statut | Détails |
| --- | --- | --- | --- | --- | --- |
| J-0 08:00 | Article dans Les Échos : "BanqueConnect : suspicion de fuite de données clients." | - Communication : Préparation d’un communiqué de crise. 
- Juridique : Vérification des obligations DORA (délai 4h). | Communication + Juridique | 🔴 Crise médiatique. | Source : Journaliste a contacté le service presse. Action : Ne pas confirmer avant validation interne. |
| J-0 09:00 | Réunion COMEX d’urgence | - Options discutées : 
1. Couper l’API Open Banking (impact : -2M€/jour). 
2. Maintenir l’API avec surveillance renforcée (risque : nouvelle fuite). 
3. Notifier l’ACPR sous 24h (obligation DORA). 
- Décision : Maintenir l’API + surveillance renforcée + notification ACPR sous 4h. | COMEX (CEO + RSSI + DPO + Directeur Com) | ✅ Décision prise. | Matrice d’arbitrage et Compte-rendu COMEX. |
| J-0 10:00 | Notification interne : Email aux employés : "Incident de sécurité en cours. Suivez les consignes du SOC." | - Cible : Tous les employés. 
- Message : Transparent mais rassurant. | Communication interne | ✅ Envoyé. | Template communncation interne |
| **J-0 11:00** |  **Début officiel de la gestion de crise (aligné DORA/RGPD).** 
- Confirmation exfiltration données sensibles clients.
- Classement incident majeur.
| - Activation : 
1. Cellule de crise (SOC + Juridique + Communication + COMEX). 
2. Procédure DORA : Classification incident majeur → délai 4h pour notifier l’ACPR. 
3. Procédure RGPD : Préparation de la notification aux clients (délai sous 72h). | RSSI (Coordinateur) | ✅ Lancé. | Procédures gestion incident et  notification (DORA + RGPD). Qualification formelle actée après réunion COMEX J-0 09h00. Délai J-1 12h00 → J-0 11h00 justifié par : forensic incomplet, périmètre provisoire, instance compétente non réunie avant J-0. Point susceptible d'être questionné par l'ACPR - justification documentée. |
| J-0 12:00 | Notification à l’ACPR (DORA Art. 19). | - Contenu : 
- Description de l’incident. 
- Données concernées (IBAN, SSN, emails). 
- Mesures prises (isolation KYCPro, audit en cours). 
- Délai : 1h après qualification formelle (respect du délai 4h DORA). | RSSI | ✅ Envoyé. | Procédures gestion incident et  notification (DORA). |
| J-0 14:00 | Communiqué de presse : "BanqueConnect a détecté une anomalie de sécurité. Investigation en cours." | - Ton : Transparent mais sans détails techniques. 
- Cible : Clients, médias, partenaires. | Communication externe | ✅ Publié. | - |
| J-0 16:00 | Premières réactions clients : Afflux de demandes sur Zendesk (+300%). | - Action : FAQ urgente publiée sur le site. 
- Message : "Vos fonds sont sécurisés. Changez votre mot de passe par précaution." | Support Client | ✅ FAQ en ligne. | Mise à jour FAQs. |
...
| Jour/Heure | Événement | Actions | Propriétaire | Statut | Détails |
| --- | --- | --- | --- | --- | --- |
| J+1 09:00 | Audit KYCPro : Rapport préliminaire → Faille corrigée (CVE-2026-4242). | - Actions : 
1. Patch déployé sur les serveurs KYCPro. 
2. Revue des accès : Révoquer les tokens compromis. 
3. Première estimation : 5 000 clients impactés (IBAN, emails, SSN). | SOC + KYCPro | ✅ Faille corrigée. | Preuve : Rapport audit KYCPro J+1. Note : KYCPro minimise toujours l’ampleur de l’incident. |
| J+1 10:00 | Envoi notification CNIL (RGPD Art. 33) | Contenu : Description de l’incident (faille KYCPro CVE-2026-4242), données concernées (5 000 clients : IBAN, emails, SSN), mesures prises (isolation KYCPro, audit en cours). Délai : 72h après confirmation (J-0 11:00 → J+1 10:00). | DPO | ✅ Envoyé. | Template défini dans procédure gestion de crise et notification. |
| J+1 14:00 | Notification aux clients - 1ère vague (RGPD Art. 34). | - Cible : 5 000 clients impactés. 
- Contenu : 
- Données exposées (IBAN, emails). 
- Mesures prises (correction de la faille). 
- Recommandations : Changer son mot de passe. 
- Canal : Email | Communication + DPO | ✅ Envoyé. | Template défini dans procédure gestion de crise et notification. |
| J+2 09:00 | Réunion de débriefing à chaud (chronologie, premières observations). | - Points abordés : 
1. Chronologie : Retard KYCPro, détection tardive. 
2. Premières observations : 
- Faille KYCPro (CVE-2026-4242). 
- Délai de détection : 46h (trop long). 
3. Actions immédiates : 
- Monitoring : Maintenir la surveillance des APIs et des données exfiltrées. 
- Contrats : Évaluer la conformité DORA dans les contrats avec les fournisseurs. 
4. Planification : REX complet prévu à J+30. | COMEX + SOC + Juridique | ✅ Débriefing documenté. | Débriefing à chaud |
| J+2 10:00 | Nouvelle découverte forensic : 7 000 clients supplémentaires impactés. | - Cause : Analyse approfondie des logs KYCPro révèle des accès non détectés entre J-5 et J-3. 
- Nouveau périmètre : 12 000 clients (vs 5 000 initialement). 
- Données supplémentaires : Historiques de transaction (pour 2 000 clients). | SOC + Forensic (cabinet externe) | 🔴 Incident élargi. | Preuve : Rapport forensic-j+2 Note : KYCPro conteste ces résultats. |
| J+2 12:00 | Rapport intermédiaire ACPR (DORA Art. 19.2). | - Contenu : 
- Chronologie mise à jour (12 000 clients). 
- Causes racines : Faille KYCPro + délai de détection. 
- Mesures correctives : Patch, audit, monitoring. 
- Délai respecté : dans les 72h après classification (J-0 11:00 → J+2 12:00). | Compliance | ✅ Envoyé. | Rapport intermediaire ACPR. |
| J+2 14:00 | Réunion COMEX : Gestion du périmètre élargi. | - Décisions : 
1. Notifier les 7 000 nouveaux clients sous 72h (RGPD), sous 72h à partir de la découverte forensique (J+2 10:00).
2. Préparer un 2ème communiqué de presse (transparence). 
3. Évaluer le remplacement de KYCPro (Onfido/Sumsub). 
- Tensions : 
- Juridique : "On doit notifier les 7 000 nouveaux clients sous 72h." 
- Communication : "Un 2ème communiqué va relancer la panique." 
- COMEX : "On notifie, mais on minimise l’impact dans le communiqué." | COMEX + Juridique + Communication | ✅ Décision prise. | Compte-rendu Comex J+2 |
| J+3 09:00 | 2ème vague de notifications clients (RGPD Art. 34). | - Cible : 7 000 nouveaux clients (périmètre élargi). 
- Contenu : 
- Données exposées : IBAN, emails, historiques de transaction. 
- Mesures prises : Audit en cours, renforcement des contrôles. 
- Canal : Email | Communication + DPO | ✅ Envoyé. | Notidication client. Note : 30% des clients contactent le support. |
| J+3 10:00 | 2ème communiqué de presse (transparence sur l’élargissement). | - Contenu : 
- "Mise à jour : L’investigation a révélé que 12 000 clients (vs 5 000 initialement) sont potentiellement impactés." 
- "Les mesures de sécurité ont été renforcées." 
- Ton : Transparent mais rassurant. | Communication | ✅ Publié. | Communiqué de presse |
| J+3 11:00 | Mise à jour notification CNIL (RGPD Art. 33) | Contenu : Extension du périmètre à 12 000 clients (découverte forensic J+2), ajout des historiques de transaction (2 000 clients). Justification : Nouvelle faille (CVE-2026-4243) confirmée par Mandiant. | DPO | ✅ Envoyé. | Note : Notification complémentaire sous 72h après découverte. |
| J+3 14:00 | KYCPro conteste le rapport forensic : "Les 7 000 clients supplémentaires ne sont pas liés à notre faille." | - Actions : 
1. SOC : Réanalyse des logs avec un cabinet externe (Mandiant). 
2. Juridique : Préparation d’un recours contractuel contre KYCPro. 
3. COMEX : Menace de résiliation du contrat si KYCPro ne coopère pas. | SOC + Juridique + COMEX | ⚠️ Conflit en cours. | Email de KYCPro : "Nous contestons votre analyse et exigeons une contre-expertise." |
| J+4 09:00 | Contre-expertise forensic (cabinet Mandiant). | - Résultats : 
- Confirmation : 12 000 clients impactés (dont 7 000 via une 2ème faille non corrigée par KYCPro). 
- Nouvelle faille : CVE-2026-4243 (non patchée par KYCPro). | SOC + Mandiant | 🔴 Nouvelle faille détectée. | Rapport Contre-exoertise Mandiant. |
| J+4 11:00 | Réunion COMEX : Décision sur KYCPro. | - Options : 
1. Maintenir KYCPro (avec pénalités contractuelles). 
2. Remplacer KYCPro par Onfido (coût : 50k€/an). 
- Décision : Remplacer KYCPro sous 30 jours. 
- Tensions : 
- Juridique : "Résilier le contrat avec KYCPro pour manquement." 
- RSSI : "Migration vers Onfido = 2 mois de travail." 
- COMEX : "On résilie, mais on négocie un délai de transition." | COMEX + Juridique + DevOps | ✅ Décision prise. | Compte-rendu COMEX J+4. |
| J+4 14:00 | Début de la migration vers Onfido. | - Actions : 
1. Contrat signé avec Onfido (clauses DORA incluses). 
2. Intégration technique : Tests des APIs Onfido. 
3. Formation des équipes : Session sur les nouvelles procédures KYC. | DevOps + KYCPro (ancien) + Onfido | 🔄 En cours. | Échéance : Migration complète sous 30 jours. |
| J+5 10:00 | KYCPro envoie une mise en demeure : "Vos allégations sont diffamatoires. Nous engageons des poursuites." | - Actions : 
1. Juridique : Préparation de la défense (preuves forensiques). 
2. COMEX : Stratégie de communication (ne pas commenter le litige en cours avec KYCPro dans les communications futures). | Juridique + COMEX | ⚠️ Litige en cours. | Preuve : Email de KYCPro : "Nous vous mettons en demeure de retirer vos allégations." |
| **J+6 09:00** | **Clôture provisoire de la cellule de crise.** | - Actions : 
1. Bilan final : 12 000 clients notifiés, aucune nouvelle exfiltration détectée à ce stade. 
2. Désactivation de la cellule de crise (mais veille renforcée maintenue). 
3. Plan de transition : Migration vers Onfido + audit de tous les fournisseurs. | COMEX | ✅ Clôture actée. | Bilan final. |
| J+7 14:00 | Réunion de clôture avec KYCPro (dernière tentative de médiation). | - Points abordés : 
1. Preuves forensiques : Mandiant confirme les 2 failles. 
2. Responsabilité : KYCPro refuse de reconnaître la 2ème faille (CVE-2026-4243). 
3. Décision : Poursuites judiciaires engagées. | Juridique + COMEX + KYCPro | ⚠️ Litige persistant. | Note : KYCPro quitte la réunion sans accord. |
| J+30 | Rapport final ACPR + REX complet | Périmètre final consolidé, causes racines, plan correctif | Compliance + RSSI + DPO | 🔄 Prévu |
